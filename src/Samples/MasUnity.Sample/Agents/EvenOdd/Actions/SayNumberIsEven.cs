...
namespace MasUnity.Sample.Agents.EvenOdd.Actions
{
    ...
    {
        private EvenOrOddRecognition Recognition { get; }
        public SayNumberIsEven(EvenOrOddRecognition recognition) => Recognition = recognition;
        ...
        public Task<Perception> Realize(CancellationToken cancellation)
        {
            ...
            return Perception.Assertion(
                ("Number is greater than 0", Recognition.IsPositiveNumber(number)),
                ("Number is Even", Recognition.IsEven(number))
            );
        }
